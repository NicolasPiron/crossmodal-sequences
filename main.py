...
def run(debugging=False, ignore_q=False):
    ...
    tools = { # store some variables there because they will be used in several functions
        'debugging': debugging,
        'pport': pport,
        'logger': logger,
        'win': win,
    ...
    # store items about the current state of the experiment
    tracker = {
        'data': [], 
        'block_id': 0, 
        'trial_id': 0, 
    ...
    try:
        ...
        for i in range(pm.n_blocks): # 4 blocks in a run
            ...
            for j in range(pm.n_trials): # 3 trials for a block
                ...
                for m, seq_name in enumerate(trial_seq_org[0:3]):
                    ...
                    pd.DataFrame(tracker['data']).to_csv(f"{out_dir}/sub-{exp_info['ID']}_run-{exp_info['run']}.csv", index=False)
                    core.wait(0.5)
        ...
        fl.log_exceptions(f'An error occurred during the run {exp_info["run"]}, block {tracker["block_id"]}: {exc}', 
            logger, 
            win)
...
def run_and_question():
    exp_info, seed, win = run(debugging=False, ignore_q=False)
    clear_stuff(win)
